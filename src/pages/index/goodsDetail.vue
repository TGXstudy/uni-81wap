<template>
	<view class="container have-fixed-bottom">
		...
		<uni-popup ref="specs" :type="'bottom'" class="popup"  :maskClick="false" >
			<view class="specs-wrapper pd-lr-30 flex-column">
				...
				<view class="specs-list-box flex-column">
					<view class="specs-item-box flex-column" v-for="(option, idx) in sku" :key="idx">
						<text class=" fn-sz-28 fn-cl-333 line-height-80">{{option.key}}： </text>
						<view class="">
							<text v-for="(item, i) in option.sku" :key="i" :class="['specs-item', item.stock > 0 ? item.is_current ? 'current-item' : '' : 'dis-item'  ]"
							 @tap="getSpec(option, item, i, idx)">{{item.name}}</text>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		watch: {
			sku: function(newval) {
				let curArr = [];
				this.sku && this.sku.forEach((opt, i) => {
					let flag = false;
					opt.sku.forEach((item, j) => {
			...
			curArr: function() {
				...
				this.specs.forEach((opt) => {
					let is_this = true;
					this.curArr.forEach((name, i) => {
						if (opt.attr_key['key' + i] != name) {
							is_this = false;
							return;
						}
					});
				...
				if (this.role == 'supplier' || this.role == 'channeler') {
					this.selectNum = this.curSpec.start_num;
				}
			}
		},
		onLoad(options) {
		...
		methods: {
			...
			getProductInfo(status = '') {
				this.$api.getProductInfo({
					...
					if (res.code == 200) {
						...
						this.specs = res.data.options;
						this.shop = res.data.shop;
						this.goodsContent = this.$mUtils.formatRichText(this.goods.content);
						this.parameter = this.goods.parameter;
						let sku = res.data.product.option_names;
						sku && sku.forEach(function(s, i) {
							let _s = [];
							s.val.forEach(function(name) {
								_s.push({
									name,
									"stock": 1,
									'is_current': false
			...
			getSpec(option, item, i, idx) {
				...
				this.curIdx = idx;
				// 当前 规格加入已选中
				this.sldIdx.push(idx);
				this.firstIdx = this.firstIdx < 0 ? idx : this.firstIdx;

				let sku = this.sku;
				this.sku = [];
				//循环sku
				sku.forEach((opt, _idx) => {
					if (_idx == idx) { // 是选择的那个sku 循环选项, 选中当前 取消其他
						
						opt.sku.forEach((item, _i) => {
							if (i == _i) {
						...
						// 不是当前选的规格,遍历选项 查询库存, 没有库存的置空
						
						if (this.firstIdx != _idx) {
							
							opt.sku.forEach((item, _i) => {
								let stock = this.getSkuStock(_idx, item.name);
								if (stock <= 0) {
									item.is_current = false;
								}
								item.stock = stock;
			...
			getCurSkuInfo(curname) {
				let res = [];
				this.specs.forEach((op, i) => {
					op.attr_key.forEach((name, k) => {
						console.log(name, k);
					});
				});
				return res;
			},
			getSkuStock(_idx, name) {
				let stock = 0;
				this.specs.forEach((op, i) => {
					if (op.attr_key['key' + this.curIdx] == this.curName && op.attr_key['key' + _idx] == name) {
						stock = op.stock;
					}
				});
				return stock;
			...
			//遍历规格库存数组 spesc
			getSkuInfo(idx, attrKey, value) {
				this.curSpecText.forEach((item, index) => {
					if (idx != index) {
						item = ""
					}
				});
			...
			confirmSpec() {
				...
				console.log("确定", this.curSpec,this.maxNum);
				if (!this.curSpec.option_id) {
					this.$mUtils.toast("请选择规格");
					return;
				}
				
				if(this.curSpec.stock < this.selectNum){
					this.$mUtils.toast("库存不足");
					return;
				}
			...
			closeSpecs() {
				...
				this.selectNum = 1;
				this.sku && this.sku.forEach(option => {
